...
#define OSAL_NV_PAGE_TO_PTR(pg) \
  ((uint8 *)((uint8 *)((HAL_NV_PAGE_BEG + ((pg) * OSAL_NV_PHY_PER_PG)) * HAL_FLASH_PAGE_SIZE)))
...
static void setPageUse( uint8 pg, uint8 inUse )
{
  ...
  flashWrite(OSAL_NV_PAGE_TO_PTR(pg)+OSAL_NV_PAGE_HDR_OFFSET, OSAL_NV_HDR_HALF, (uint8 *)(&pgHdr));
}
...
static uint8 compactPage( uint8 srcPg, uint16 skipId )
{
  uint16 srcOff = OSAL_NV_PAGE_HDR_SIZE;
  uint8 rtrn = TRUE;
...
static void setItem( uint8 pg, uint16 offset, eNvHdrEnum stat )
{
  ...
  else // if ( stat == eNvZero )
  {
    uint16 sz = ((hdr.len + (OSAL_NV_WORD_SIZE-1)) / OSAL_NV_WORD_SIZE) * OSAL_NV_WORD_SIZE +
                                                                          OSAL_NV_HDR_SIZE;
    hdr.id = 0;
    flashWrite(OSAL_NV_PAGE_TO_PTR(pg) + offset, OSAL_NV_HDR_ITEM, (uint8*)(&hdr));
    pgLost[pg] += sz;
  }
}
...
static uint8 writeItem( uint8 pg, uint16 id, uint16 len, void *buf, uint8 flag )
{
  ...
  flashWrite(OSAL_NV_PAGE_TO_PTR(pg) + offset, OSAL_NV_HDR_HALF, (uint8 *)(&hdr));
  readHdr( pg, offset, (uint8 *)(&hdr) );
