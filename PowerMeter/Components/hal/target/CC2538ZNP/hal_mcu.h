...
/* SoftReset for USB . To maintain USB communication through 
 * reset, do not reset GPIO C Pin 0 (USB D+) and USB registers. 
 */
#define SOFT_RESET 0xABCDABCD
      
/* Hard reset . USB communication will not survive hard reset */
#define HARD_RESET 0xFFFFFFFF
            
      
extern uint32 softReset; 
#define HAL_SYSTEM_RESET()  \
do {                        \
  softReset = HARD_RESET;   \
  SysCtrlReset();           \
}while(0)
#else
#define HAL_SYSTEM_RESET()  \
